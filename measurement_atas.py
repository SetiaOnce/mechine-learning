...
camera = cv2.VideoCapture('Videos/dimention_atas2.MOV')
# Model PyTorch
...
while True:
    ...
    if not ret or frame_count % skip_frames != 0:
        ...
        frame = cv2.resize(frame, (536, 874), interpolation=cv2.INTER_LINEAR)
        ...
        if len(results.boxes) > 0:
            ...
            for res in results.boxes.data.tolist():
                ...
                if currentClass == 'atas' and reference_pixel_height is not None:
                    # Gambar area polygon
                    overlay = frame.copy()
                    ...
                    cvzone.putTextRect(frame, f'Lebar: {object_width_cm:.2f} cm', (20, 50), colorR=(0, 255, 0), scale=2,
